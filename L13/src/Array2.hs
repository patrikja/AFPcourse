...
class ArrayElem a where
  -- The type signature for our associated datatype
  data Array a
...
instance ArrayElem Int where
  newtype Array Int       = ArrInt SuperEfficientByteArray 
...
instance ArrayElem Size where
  newtype Array Size = ArrSize (Array Int)

instance ArrayElem Index where
  newtype Array Index = ArrIndex (Array Int)
...
instance (ArrayElem a, ArrayElem b) => ArrayElem (a, b) where
  data Array (a, b)    = ArrPair (Array a) (Array b)
...
instance ArrayElem a => ArrayElem (Array a) where
  ...
  data Array (Array a) = ArrNested (Array a) (Array (Index, Size))
...
instance Show (Array Size) where
  show (ArrSize a) = show a

instance Show (Array Index) where
  show (ArrIndex a) = show a
