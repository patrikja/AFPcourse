...
-- | Alternative implementation as two mutually recursive types
data ExprI where
  LitN  :: Int                             -> ExprI
  (:+)  ::              ExprI   -> ExprI   -> ExprI
...
--
-- Instead we define a type of proofs that two types @a@ and @b@
-- are equal.  The only way to prove two types equal is if they
-- are in fact the same, and then the proof is Refl. Evaluating
-- one of these proofs to 'Refl' will convince GHC's type
-- checker that the two type arguments are indeed equal (how
-- else could the proof be Refl?).
